 ...
 * @private
 */
const _cookieName:string = '__debug__';
...
let _registeredNamespaces:Array<string> = [];
...
let _cachedCookieValue:string;
let _cachedGlobRegExp:RegExp;
...
class Log
{
	...
	{
		return /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor); // currently only chrome supports it
	}
	...
	private static getConsolePrefix(namespace:string, features:ILogFeatures, lastLogTime?:number):string
	{
		...
		if(features.date == true || features.time == true)
		{
			...
			var month = padStart((date.getMonth() + 1).toString(), 2, '0');
			var day = padStart(date.getDate().toString(), 2, '0');

			var hours = padStart(date.getHours().toString(), 2, '0');
			var minutes = padStart(date.getMinutes().toString(), 2, '0');
			var seconds = padStart(date.getSeconds().toString(), 2, '0');
